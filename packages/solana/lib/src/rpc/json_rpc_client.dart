...
class JsonRpcClient {
  ...
  Future<List<Map<String, dynamic>>> bulkRequest(
    ...
    if (response is _JsonRpcArrayResponse) {
      ...
      throw FormatException('unexpected jsonrpc response type');
    }
  }
  ...
  Future<_JsonRpcResponse> _postRequest(
    ...
    final http.Response response = await http
        ...
        .timeout(_timeout, onTimeout: JsonRpcClient._onTimeout);
    // Handle the response
      ...
      return _JsonRpcResponse._array(
        response.map((r) {
          if (r is Map<String, dynamic>) {
            ...
            throw FormatException('cannot parse the jsonrpc response');
          }
        }).toList(growable: false),
      );
