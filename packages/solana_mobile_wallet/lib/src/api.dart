...
abstract class ApiFlutter {
  ...
  Future<WalletConfigDto?> getWalletConfig(int id); // P46ea
  ...
  static void setup(ApiFlutter? api, {BinaryMessenger? binaryMessenger}) {
    ...
    {
      final BasicMessageChannel<Object?> channel = BasicMessageChannel<Object?>(
          'dev.flutter.pigeon.ApiFlutter.getWalletConfig', codec,
          binaryMessenger: binaryMessenger); // P46ea
      if (api == null) {
        channel.setMessageHandler(null);
      } else {
        channel.setMessageHandler((Object? message) async {
          assert(message != null,
              'Argument for dev.flutter.pigeon.ApiFlutter.getWalletConfig was null.');
          final List<Object?> args = (message as List<Object?>?)!;
          final int? arg_id = (args[0] as int?);
          assert(arg_id != null,
              'Argument for dev.flutter.pigeon.ApiFlutter.getWalletConfig was null, expected non-null int.');
          final WalletConfigDto? output =
              await api.getWalletConfig(arg_id!); // Pbd8f
          return output;
        });
      }
    }
...
class ApiHost {
  ...
  Future<WalletConfigDto?> getWalletConfig(int arg_id) async { // P0561
    final BasicMessageChannel<Object?> channel = BasicMessageChannel<Object?>( // P0561
        'dev.flutter.pigeon.ApiHost.getWalletConfig', codec, // P0561
        binaryMessenger: _binaryMessenger); // P0561
    final List<Object?>? replyList = // P0561
        await channel.send(<Object?>[arg_id]) as List<Object?>?; // P0561
    if (replyList == null) { // P0561
      throw PlatformException( // P0561
        code: 'channel-error', // P0561
        message: 'Unable to establish connection on channel.', // P0561
      ); // P0561
    } else if (replyList.length > 1) { // P0561
      throw PlatformException( // P0561
        code: replyList[0]! as String, // P0561
        message: replyList[1] as String?, // P0561
        details: replyList[2], // P0561
      ); // P0561
    } else { // P0561
      return (replyList[0] as WalletConfigDto?); // P0561
    } // P0561
  } // P0561
}
