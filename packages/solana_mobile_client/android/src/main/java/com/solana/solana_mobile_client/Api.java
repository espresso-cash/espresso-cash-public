...
public class Api {
  ...
  public interface ApiLocalAssociationScenario {
    ...
    @NonNull 
    Boolean isWalletEndpointAvailable();
    ...
    static void setup(BinaryMessenger binaryMessenger, ApiLocalAssociationScenario api) {
      ...
      }
      {
        BasicMessageChannel<Object> channel =
            new BasicMessageChannel<>(
                binaryMessenger, "dev.flutter.pigeon.ApiLocalAssociationScenario.isWalletEndpointAvailable", getCodec());
        if (api != null) {
          channel.setMessageHandler(
              (message, reply) -> {
                ArrayList wrapped = new ArrayList<>();
                try {
                  Boolean output = api.isWalletEndpointAvailable();
