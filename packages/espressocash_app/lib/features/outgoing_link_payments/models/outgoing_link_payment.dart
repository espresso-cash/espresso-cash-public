...
class OLPStatus with _$OLPStatus {
  ...
  const factory OLPStatus.txCreated(SignedTx tx, {required EscrowPrivateKey escrow}) = OLPStatusTxCreated;

  /// Tx sent sent to backend. Should be good as confirmed at this point
  const factory OLPStatus.txSent(SignedTx tx, {required EscrowPrivateKey escrow, required String signature}) =
      OLPStatusTxSent;

  /// Link is ready to be sent to the recipient.
  const factory OLPStatus.linkReady({required Uri link, required EscrowPrivateKey escrow}) = OLPStatusLinkReady;

  /// Money are withdrawn from the escrow by someone, but not by the sender. The
  /// payment is complete.
  const factory OLPStatus.withdrawn({required String txId, required DateTime? timestamp}) = OLPStatusWithdrawn;

  /// Money are withdrawn by the sender. The payment is complete.
  const factory OLPStatus.canceled({required String? txId, required DateTime? timestamp}) = OLPStatusCanceled;
  ...
  const factory OLPStatus.cancelTxCreated(SignedTx tx, {required EscrowPrivateKey escrow}) = OLPStatusCancelTxCreated;

  /// There was an error while creating the cancellation tx, or the tx was
  /// rejected. It's safe to recreate it.
  const factory OLPStatus.cancelTxFailure({required TxFailureReason reason, required EscrowPrivateKey escrow}) =
  ...
  const factory OLPStatus.cancelTxSent(SignedTx tx, {required EscrowPrivateKey escrow, required String signature}) =
