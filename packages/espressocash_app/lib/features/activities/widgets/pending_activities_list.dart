...
class _PendingActivitiesListState extends State<PendingActivitiesList> {
  ...
  Widget build(BuildContext context) => StreamBuilder<IList<Activity>>(
    ...
    builder: (context, snapshot) {
      ...
      final pendingActivities = data.toList();
                    ...
                    itemBuilder: (context, index) {
                      ...
                      return item.map(
                        outgoingPaymentRequest: (p) =>
                            PaymentRequestTile(key: ValueKey(p.id), id: p.id),
                        ...
                        outgoingDlnPayment: (it) =>
                            OutgoingDlnTile(key: ValueKey(it.id), activity: it),
                        transactionRequest: (it) => TrTile(key: ValueKey(it.id), activity: it),
                        swap: (it) => SwapTile(key: ValueKey(it.id), activity: it),
                      );
                    },
                  ),
                ),
