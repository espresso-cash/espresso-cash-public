...
class SwapTile extends StatelessWidget {
  const SwapTile({super.key, required this.activity, this.showIcon = true});
  ...
  Widget build(BuildContext context) {
    ...
    final swapTo = data.output.maybeFlatMap((it) => it.format(locale, maxDecimals: 2));
...
class _SwapIcon extends StatelessWidget {
  const _SwapIcon({required this.input, required this.output});
