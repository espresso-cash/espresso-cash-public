...
extension MyDatabaseExt on MyDatabase {
  Future<OnRampOrderRow?> getNonCompletedOnRampOrder(String orderId) {
    final query = select(onRampOrderRows)..where((tbl) => tbl.id.equals(orderId) & tbl.isCompleted.equals(false));
  ...
  Future<OffRampOrderRow?> getOffRampOrderWithStatus(String orderId, {required OffRampOrderStatus status}) {
    final query = select(offRampOrderRows)..where((tbl) => tbl.id.equals(orderId) & tbl.status.equals(status.name));
