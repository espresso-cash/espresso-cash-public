...
class OnRampOrderService implements Disposable {
  ...
  Future<void> confirmDeposit(String orderId) async {
    ...
    switch (order.status) {
      case OnRampOrderStatus.waitingForDeposit:
        await updateQuery.write(const OnRampOrderRowsCompanion(status: Value(OnRampOrderStatus.waitingForPartner)));
  ...
  Stream<OnRampOrder> watch(String id) {
    ...
    return query.watchSingle().asyncMap((row) async {
      ...
      final submittedAmount =
          isFiat
              ? FiatAmount(value: row.amount, fiatCurrency: currencyFromString(row.fiatSymbol ?? 'USD'))
              : CryptoAmount(value: row.amount, cryptoCurrency: CryptoCurrency(token: token ?? Token.unk));
      ...
      return (
        ...
        receiveAmount: row.receiveAmount?.let(
          (amount) => CryptoAmount(value: amount, cryptoCurrency: CryptoCurrency(token: token ?? Token.unk)),
        ),
        partner: row.partner,
        status: row.status,
        ...
        manualDeposit:
            isManualDeposit
                ? (
                  ...
                  transferAmount: FiatAmount(value: transferAmount, fiatCurrency: currencyFromString(fiatSymbol)),
                )
                : null,
        authToken: row.authToken,
        additionalDetails: (fee: feeAmount, moreInfoUrl: moreInfoUrl, referenceNumber: row.referenceNumber),
      );
    });
  }
  ...
  void _subscribe(String orderId) {
    _subscriptions[orderId] = Stream<void>.periodic(const Duration(seconds: 5))
        .asyncMap((_) async {
          final order = await (_db.select(_db.onRampOrderRows)..where((tbl) => tbl.id.equals(orderId))).getSingle();
          ...
          if (expiry != null && expiry.isBefore(DateTime.now())) {
            return OnRampOrderRowsCompanion(id: Value(order.id), status: const Value(OnRampOrderStatus.depositExpired));
          }
        })
        .whereNotNull()
        .listen((event) async {
          ...
          await (_db.update(_db.onRampOrderRows)..where((tbl) => tbl.id.equals(orderId))).write(event);
        });
  }
