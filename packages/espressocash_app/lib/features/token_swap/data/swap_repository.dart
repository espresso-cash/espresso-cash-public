...
class SwapRepository {
  ...
  Future<List<Swap>> getAllPending() {
    ...
    return query.get().then((rows) => Future.wait(rows.map((row) => row.toModel())));
  }
  ...
  Stream<List<Swap>> watchAllPending() {
    ...
    return query.watch().asyncMap((rows) => Future.wait(rows.map((row) => row.toModel())));
  }
  ...
  Stream<IList<Swap>> watchTxCreated() => _watchWithStatuses([SwapStatusDto.txCreated]);

  Stream<IList<Swap>> watchTxSent() => _watchWithStatuses([SwapStatusDto.txSent]);

  Stream<IList<Swap>> _watchWithStatuses(Iterable<SwapStatusDto> statuses) {
    final query = _db.select(_db.tokenSwapRows)..where((p) => p.status.isInValues(statuses));
...
enum SwapStatusDto { txCreated, txSent, success, txFailure }

enum SlippageDto { zpOne, zpFive, onePercent }
    ...
    data: SwapData(
      input: CryptoAmount(
        ...
        cryptoCurrency: CryptoCurrency(
          token: (await sl<TokenRepository>().getToken(inputMint)) ?? Token.unk,
        ),
      ),
      output: CryptoAmount(
        value: outputAmount,
        cryptoCurrency: CryptoCurrency(
          token: (await sl<TokenRepository>().getToken(outputMint)) ?? Token.unk,
        ),
      ),
...
extension on SwapStatus {
  ...
  String? toTxId() =>
      mapOrNull(txSent: (it) => it.tx.id, success: (it) => it.tx.id, txCreated: (it) => it.tx.id);
