...
class TokenSwapService {
  ...
  Future<Swap> create({required ECWallet account, required SwapRoute route}) async {
  ...
  Future<SwapStatus> _createTx({required SwapRoute route, required ECWallet account}) async {
    final tx = await route.encodedTx.let(SignedTx.decode).let((it) => it.resign(account));
  ...
  Future<Swap> _wait(Swap swap) async {
    ...
    final tx = await _txSender.wait(status.tx, minContextSlot: status.slot, txType: 'Swap');
