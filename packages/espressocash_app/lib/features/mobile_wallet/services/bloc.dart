  ...
  void _onDeclined(Emitter<RemoteRequestState> emit) {
    ...
    final result = request.map(
      ...
      signTransactionsForSending: always(
        const SignaturesResult.requestDeclined(),
      ),
    );
...
extension on _ValidationError {
  SignaturesResult toSignaturesResult() => when(
    ...
    authorizationNotValid: always(
      const SignaturesResult.authorizationNotValid(),
    ),
  );
  ...
  SignedPayloadResult toSignedPayloadResult() => when(
    ...
    authorizationNotValid: always(
      const SignedPayloadResult.authorizationNotValid(),
    ),
  );
}
...
Uint8List _buildScope() => [
  _scopeTag,
  _qualifier,
].whereType<String>().join(',').let(utf8.encode).let(Uint8List.fromList);
