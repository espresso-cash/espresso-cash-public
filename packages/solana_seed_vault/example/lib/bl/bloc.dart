...
class SeedVaultBloc extends Cubit<SeedVaultState> {
  ...
  Future<void> init() async {
    final isInstalled = await SeedVault.instance.isAvailable(allowSimulated: true);
    ...
    if (granted) {
      _subscription = SeedVault.instance.notificationStream.listen((_) => refreshUI());
  ...
  Future<void> refreshUI() async {
    ...
    final limits = await SeedVault.instance.getParsedImplementationLimitsForPurpose(purpose);
    ...
    emit(
      SeedVaultState.loaded(
        ...
        hasUnauthorizedSeeds: await SeedVault.instance.hasUnauthorizedSeedsForPurpose(purpose),
      ),
    );
  }

  AsyncResult<String> signMessageWithAccount(AuthToken authToken, Account account) => _signMessages(
    authToken: authToken,
    signingRequests: [
      SigningRequest(
  ...
  AsyncResult<String> signTransactionWithAccount(AuthToken authToken, Account account) =>
      _signTransactions(
        authToken: authToken,
        signingRequests: [
          SigningRequest(
  ...
  AsyncResult<String> signTransactions(AuthToken authToken) async => _signTransactions(
    authToken: authToken,
    signingRequests: await _generateSigningRequests(
      authToken: authToken,
      payloadCount: _maxSigningRequests,
  ...
  AsyncResult<List<String>> requestPublicKeys(AuthToken authToken) =>
      _requestPublicKeys(authToken, _generateUris(_maxRequestedPublicKeys));

  AsyncResult<String> exceedMaxSigningRequests(AuthToken authToken) async => _signTransactions(
    authToken: authToken,
    signingRequests: await _generateSigningRequests(
      authToken: authToken,
  ...
  AsyncResult<String> exceedMaxRequestedSignatures(AuthToken authToken) async => _signTransactions(
    authToken: authToken,
    signingRequests: await _generateSigningRequests(
      authToken: authToken,
      payloadCount: 1,
  ...
  AsyncResult<List<String>> exceedMaxRequestedPublicKeys(AuthToken authToken) =>
      _requestPublicKeys(authToken, _generateUris(_maxRequestedPublicKeys + 1));
  ...
  AsyncResult<void> authorizeSeed() => tryEitherAsync((_) async {
    await SeedVault.instance.authorizeSeed(Purpose.signSolanaTransaction).letAsync(_onNewSeed);
  });

  AsyncResult<void> importSeed() => tryEitherAsync((_) async {
    await SeedVault.instance.importSeed(Purpose.signSolanaTransaction).letAsync(_onNewSeed);
  });

  AsyncResult<void> createSeed() => tryEitherAsync((_) async {
    await SeedVault.instance.createSeed(Purpose.signSolanaTransaction).letAsync(_onNewSeed);
  });
  ...
  Future<void> _onNewSeed(AuthToken authToken) async {
    for (int i = 0; i < _accountsPerSeed; i++) {
      final derivationPath = Bip44DerivationPath.toUri([BipLevel(index: i, hardened: true)]);
      ...
      final account = await SeedVault.instance
          .getParsedAccounts(authToken, filter: AccountFilter.byDerivationPath(resolvedPath))
  ...
  AsyncResult<List<String>> _requestPublicKeys(AuthToken authToken, List<Uri> uris) =>
      tryEitherAsync((_) async {
  ...
  List<Uri> _generateUris(int count) =>
      List.generate(count, (i) => Bip32DerivationPath.toUri([BipLevel(index: i, hardened: true)]));
